...
function getDecoder() {
  ...
  return {
    write: function(chunk) {
      ...
      if (typeof decoder.end === "function")
        chunk = decoder.end();
...
// If you supply non-`utf8` encoded multi-byte data, it will not work correctly.
module.exports = function normalize(format) {
  ...
  ts._transform = function(chunk, encoding, callback) {
    chunk = decoder.write(chunk);
    if (chunk)
      this.push(chunk.replace(find, format));
    callback();
  };
